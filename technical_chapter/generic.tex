...
In the context of our work the LLVM architecture is instantiated over all LLVM frontends \step{1}, it adds a Diversifier as a LLVM IR pass \step{2} and uses a custom \termidx[Wasm]{WebAssembly!Programming language}backend \step{3}.
...
The diversifier generates LLVM IR variants from of the output of the frontend \step{2}. These variants are inputs for our customized \termidx[Wasm]{WebAssembly!Programming language}backend. In our case the LLVM backend is always for \wasm and, it finalizes the creation of the variants \step{3}. 
...
In addition, an orthogonal contribution comes from the generation of LLVM IR variants at Step~\step{2}. Our second contribution, \termidx{MEWE } \cite{MEWE}, merges and creates \termidx[multivariant ]{Software!Multivariant}binaries to provide MVE for \wasm \step{4}. In \autoref{section:mewe} we describe \termidx{MEWE }in details.
