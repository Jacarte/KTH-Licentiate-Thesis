...
The core idea of \tool is to synthesize diversified function variants, using CROW, providing execution-path randomization. Thus, goal of \tool is to synthesize multivariant  \wasm binaries. %, according to the threat model presented in \autoref{sec:threat-model}. 
...
In Step~\step{1}, the binary is passed to CROW, which is a superdiversifier for Wasm that generates a set of  variants for the functions in the binary. 
...
The multivariant combination (Step \step{2}) is implemented in 942 lines of C++ code.
Its uses the LLVM 12.0.0 libraries to extend the LLVM standard linker tool capability with the multivariant generation.
\tool's Mixer (Step \step{3}) is implemented as an orchestration of the rustc and the \wasm backend provided by CROW.
...
%\subsubsection*{Dangers related to Edge Computing}
...
\todo{Replace HTTP harnessing here.}


\todo{Stress the variant generation in the previous section.}
...
Step~\step{2} of \tool consists in combining the variants generated for the original functions, into a single binary.
...
define internal i32 @b64_byte2urlsafe_char(i32 %0) {
    ...
    \noindent\rule{\linewidth}{0.4pt}
    \captionof{lstlisting}{Dispatcher function embedded in the multivariant binary of the original function in \todo{} program, which corresponds to the rightmost green node in \autoref{multivariant}.}\label{listing:multivariant_template}
\end{minipage}
\end{code}

\todo{Add some words on the soundness of memory and data because of the artificial character of the generation.}
